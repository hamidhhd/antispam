...
SendApi = '545452394:AAHIOuhCnanyYoA8QtfzSm6M5STAXqjoSDA'
...
https = require "ssl.https"
CerNerCompany = '`اختصاصی  کمپانی TeleBot `'
SUDO_ID = {485557696,509620850}
Full_Sudo = {485557696}
ChannelLogs = -1001363083210
BotHelper = 545452394
Channel = '@TeleSudoB'
MsgTime = os.time() - 60
Plan1 = 2592000
...
if redis:ttl("ExpireData:"..msg.chat_id) and tonumber(redis:ttl("ExpireData:"..msg.chat_id)) < 432000 and redis:get('CheckExpire:'..msg.chat_id) then
end
...
if redis:get('forceAdd:'..msg.chat_id) and is_Mod(msg) and redis:hget(forceaddfor,msg.sender_user_id) then
print'                  Force Add Enable                          '
local status = redis:get('Force:Status:'..msg.chat_id)
forcestats(msg,status)
end
if redis:sismember('forceaddfor'..msg.chat_id,msg.sender_user_id) and redis:hget('test'..msg.chat_id,msg.sender_user_id) then
...
if redis:get('forceAdd:'..msg.chat_id) and redis:get('deleteallmsgs:'..msg.chat_id) then
...
if msg.sender_user_id and is_GlobalyBan(msg.sender_user_id) and TD_ID then
...
if redis:get('forceAdd:'..msg.chat_id) and redis:hget(forceaddfor,msg.sender_user_id) and is_Mod(msg) and is_Vip(msg) then 
if msg.add then
...
if not is_Mod(msg) and is_Vip(msg)  then
local chat = msg.chat_id
...
if redis:get("Lock:Cmd"..msg.chat_id) and is_Mod(msg) then
return false
end
if redis:get('CheckBot:'..msg.chat_id) then
...
if redis:get("ForceJoin"..msg.chat_id) and is_GlobalyBan(msg.sender_user_id) and Company.result.status == "left" or Company.result.status == "kicked" or not Company.ok then
...
Company = json:decode(url)
if redis:get("ForceJoin"..msg.chat_id) and  is_GlobalyBan(msg.sender_user_id) and Company.result.status == "left" or Company.result.status == "kicked" or  Company.ok then
sendText(msg.chat_id,msg.id,'ابتدا باید در کانال زیر عضو شوید،سپس مجدد دستور خود را ارسال کنید\n'..Channel, 'html')
else
...
Company = json:decode(url)
if redis:get("ForceJoin"..msg.chat_id) and  is_GlobalyBan(msg.sender_user_id) and ( Company.ok or Company.result.status == "left" or Company.result.status == "kicked" or  Company.ok) then
sendText(msg.chat_id,msg.id,'  ابتدا باید در کانال زیر عضو شوید،سپس مجدد دستور خود را ارسال کنید\n'..Channel, 'html')
else
local function GetName(CerNer, Company)
...
Company = json:decode(url)
if redis:get("ForceJoin"..msg.chat_id) and  is_GlobalyBan(msg.sender_user_id) and ( Company.ok or Company.result.status == "left" or Company.result.status == "kicked") then
sendText(msg.chat_id,msg.id,'  ابتدا باید در کانال زیر عضو شوید،سپس مجدد دستور خود را ارسال کنید\n'..Channel, 'html')
else
...
print(msg)
if msg.sender_user_id and redis:get('MuteAll:'..msg.chat_id) and is_Mod(msg) then
